...
// Replace the current STM32BluePill3D
...
const newCode = `export const STM32BluePill3D = ({ position, rotation, size, fp, onSelect, onHover, isSelected, isHovered }: any) => {
  ...
  const boardW = size.h || 22.9;
  const boardH = size.w || 53.3;
  const boardD = 1.6;
  ...
  return (
    <group position={position} rotation={adjustedRotation}
      ...
      {/* Visual Board Group - Rotated 180 degrees so USB and MCU align correctly with pins */}
...
console.log('Done replacing STM32BluePill3D');
