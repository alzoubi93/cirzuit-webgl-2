...
const newCode = `export const STM32BluePill3D = ({ position, rotation, size, fp, onSelect, onHover, isSelected, isHovered }: any) => {
  ...
  const boardW = isHorizontal ? size.w : size.h;
  const boardH = isHorizontal ? size.h : size.w;
  const boardD = 1.6;
  ...
  return (
    <group position={position} rotation={adjustedRotation}
      ...
      <group rotation={[0, 0, isHorizontal ? -Math.PI / 2 : 0]}>
        {/* Raised PCB */}
        <RoundedBox args={[boardW > boardH ? boardH : boardW, boardW > boardH ? boardW : boardH, boardD]} radius={0.3} smoothness={4} position={[0, 0, pcbMidZ]} castShadow>
        ...
        <group>
          ...
          <group position={[0, Math.max(boardW, boardH) / 2 - 2, pcbTopZ + 1.25]}>
            <mesh castShadow>
          ...
          {/* Reset Button */}
          <group position={[Math.min(boardW, boardH) / 2 - 3, Math.max(boardW, boardH) / 2 - 10, pcbTopZ + 0.5]}>
          ...
          <group position={[0, Math.max(boardW, boardH) / 2 - 12, pcbTopZ + 2.5]}>
            <mesh castShadow><boxGeometry args={[10, 2.54, 5]} /><meshStandardMaterial color="#ffeb3b" roughness={0.6} /></mesh>
          </group>
          
          {/* 32.768kHz Crystal */}
          <group position={[-Math.min(boardW, boardH) / 2 + 4, -5, pcbTopZ + 0.8]}>
